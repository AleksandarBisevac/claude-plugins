...
1. **Arbitrary Bash writes.** `sed -i`, `tee`, `>`/`>>` redirects (incl. heredoc
   ...
   (`python -c "open(...,'w')"`) are heuristically blocked — but full Bash-write
   coverage is statically undecidable (heredocs piped into interpreters,
   obfuscated redirects; upstream: anthropics/claude-code#29709). The complete
   control would be a PostToolUse `git status` diff check; planned, not shipped.
2. **Subagents do not inherit parent hooks** in all versions
   (anthropics/claude-code#43772). Mitigation: the `/audit` orchestrator — not
   its subagents — performs all manifest writes and commits, and subagent
   prompts restate the hard rules.
...
5. **Bypass residuals.** The single-use `#no-plan` bypass is consumed on
   ...
   *discussing* the keyword arms it (you are told when that happens).
