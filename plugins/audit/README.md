...
- **Hooks** (all launched via `py-launch.sh`, which resolves `python3` → `python` → `py`;
  ...
  - `guard-bash-writes.py` (PostToolUse: Bash + edits) — **non-blocking** git-status diff
    ...
    outside of, so it stays quiet there unless `enforce`/`planGate` says otherwise. It
    watches **one working tree** — the configured `gitRoot` — and a command that ran in
    another (an agent worktree, say) is told that rather than blamed for what is dirty
    here. Its
...
| Auth tokens are never logged via `console.*` / Sentry / `remoteLog` | `guard-edits.py` — PreToolUse `Edit\|Write\|MultiEdit\|NotebookEdit` | **deny**. Prefix-only debug (`token.slice(0, 6)`) is allowed. **Not reached through an MCP server's write tool**, and nothing replaces it: the rule grades the text that will become file bytes, and only an edit tool's fixed schema can name that text — grading every string in an MCP payload would refuse a commit message that quotes the pattern |
...
itself — a `*` .gitignore inside; self-trimming past ~512KB; writing it never blocks anything).
**`/audit:logs prune` is how rows leave it** — the plugin both writes this file and shows it,
so cleaning it is the plugin's job and not a script you have to write; it removes rows naming
a path outside this repository and lines that are not JSON, prints what went and what stayed
at every count including zero, and touches nothing outside `logsDir`. The panel's
...
/audit:bug close BUG-2 wontfix                  # real, and the fix will not be made
/audit:bug close BUG-3 not_a_bug                # looked at it; the behaviour is correct
```
...
| `buildCommands` | Map so gate entries like `test` resolve to a real command. Two more are **offered, never added**: `python3 scripts/status/_gate_feed.py --list-offered` names `provesCanFail` and `claimsMatchCode` — the checks this project's own history learnt the hard way, each with what it asks of a change and what it costs to run. `/audit:init`'s recon step asks before either lands; a plan that declines both is unchanged in every byte. | — |
...
- **A guard says something this release does not say.** `CLAUDE_PLUGIN_ROOT` is fixed when a
  session starts, so a session that began before an upgrade goes on running the copy it started
  with — however many times the plugin is updated underneath it. The hooks are the half that
  keeps running the old copy, and nothing in the session can make them reload it: **start a new
  session**. `/audit:doctor`'s `running plugin` row is how you find out, and it grades three
  ways — the copy that ran the hooks matches the one the command is running from, it does not,
  or it could not be determined (which is not the same as matching).
...
`/audit:task cancel <taskId> --reason "<why>"` for a task and
`/audit:phase cancel <phaseId> --reason "<why>"` for a phase (the task spelling still takes a
phase id, as the legacy spelling of the phase one). Either records the reason, the moment and a
