...
| `/audit:status` | `[--gate] [--fail-on <c1,c2,...>] [--phase <id>] [--json] [--color auto\|always\|never]` | Read-only rollup — phases, tasks, bugs, and the ready-now list, with per-phase progress and resumable-phase flags. **`--phase <id>` scopes the human render only** — the table lists that phase, the totals above it stay whole-plan, and neither `--gate` nor `--json` is scoped by it, so `--phase P3 --gate` still gates on the whole manifest. No locks, no mutations. |
...
curl -fsSL https://raw.githubusercontent.com/AleksandarBisevac/claude-plugins/v1.8.0/plugins/audit/templates/audit-plan.starter.json -o docs/audit/audit-plan.json
curl -fsSL https://raw.githubusercontent.com/AleksandarBisevac/claude-plugins/v1.8.0/plugins/audit/templates/audit.config.example.json -o .claude/audit.config.json   # optional
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/AleksandarBisevac/claude-plugins/v1.8.0/plugins/audit/schema/audit-plan.schema.json -o /tmp/audit-plan.schema.json
