...
def _cases(check):
    ...
    for _cmd, _want in (
            ('grep -n "cost > 5" f.py', True),
            ('grep -n "a && b" f.py', True),
            ('grep -n "${VAR}" f.py', True),
            ('grep -n "$(date)" f.py', True),
            ('grep -rn "<<EOF" f.py', True),
            ('grep -n "READ_ONLY\\|read_only" f.py | head', True),
            ('find . -name "*.py" | xargs wc -l', True),
            ('grep x f.py 2>&1', True),
            ('ls >/dev/null 2>&1', True),
            ('grep x f.py >&2', True),
            ('ls | xargs -0 grep -l x', True),
            ('sed -i.bak s/a/b/ f.py', False),
            ('sed --in-place=.bak s/a/b/ f.py', False),
            ('echo hi > f.py', False),
            ('cat a > /dev/null.bak', False),
            ('find . -exec sed -i s/a/b/ {} +', False),
            ('grep -n "unbalanced f.py', False),
            ...
            ('find . | xargs -n1 rm -f', False),
            ('ls | xargs -I{} mv {} /tmp', False),
            ('xargs', False)):
        check("rwq %r is %s" % (_cmd,
                               "provably read-only" if _want else "watched"),
              M._command_is_read_only(_cmd) == _want)
