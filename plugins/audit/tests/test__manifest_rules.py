#!/usr/bin/env python3
"""
The cases for `_manifest_rules.py` - every referential rule the manifest is held to.

THIS SUITE USED TO BE `test_validate_manifest.py` AND IT FOLLOWED ITS SUBJECT.
The rules were `validate-manifest.py`'s body until four modules that are not
commands needed them: `_panel_state` (L5), `audit-doctor`, `audit-status` and
`migrate-manifest` each reached the validator through
`_loader.load_script("validate-manifest.py")`, and `_deps.layer_violations()`
counts those calls, so four of the seventeen `KNOWN_LAYER_DEBT` entries were that
one file being loaded as a library. The rules moved to `_manifest_rules.py` at
layer 2 where all four can import them, and the cases about the rules moved with
them - a suite named after the CLI while asserting the CLI's dependency is a
suite nobody can place. What stayed in `test_validate_manifest.py` is the four
cases that are genuinely about the COMMAND: the exit codes.

`M` is the module under test, and it is a plain `import` now rather than a
`_loader.load_script` - which is the whole point of the move made visible in one
...
`_valid_manifest()` came with the suite. It sat under the validator's own
...
builder in a shipped CLI with nothing to build for. `test_validate_manifest.py`
now carries its own copy for the four exit-code cases, and that is duplication on
purpose: the two suites must be independently runnable, and a fixture shared
across test files is a third module for the harness to own.
