...
into the page the browser gets, and the ~283 selftest cases that read the result.
...
`usage_ledger` (L3, for the cost-band constant), `_help` (L3, from the selftest
only), `_panel_ui` and `_panel_settings` (L2), `_ui_theme` and `_loader` (L1), and
nothing else. It must never import `_panel_state`, `_panel_write`, `_panel_discovery`
or panel-server — a selftest case below says so — which is also why the cases that
mix a page claim with a server call (build_state, _run_status, help_field, the
route slices) stayed behind rather than dragging half the server down here.
"""
import json
import os
import re
...
# --- selftest -------------------------------------------------------------------
def _selftest():
    # `_help` is imported HERE and not at module scope on purpose: production needs
    # none of it, and this file is imported by a server whose start-up cost is paid
    # on every /audit:panel. It is still a real edge (_deps reads function bodies
    # too) and a downward one - _help is layer 3, this file is layer 4.
    import _help
    ...
    check("pg1 UI_TEMPLATE is UI_HTML with ONLY the theme block left as a marker: "
          ...
          UI_TEMPLATE.replace("/*__THEME_TOKENS__*/", _theme.TOKEN_CSS) == UI_HTML
