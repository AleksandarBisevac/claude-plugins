...
| **Monorepo areas** (tags + registry) | `P1: auth` · `P2: storefront, checkout` (a multi-tag phase) · `P3: storefront` · `BF1` untagged on purpose · `P5: worker`, a different part of the stack with its own gate vocabulary (`go test` rather than `npm test`) · described in `meta.areas` — the report's Area chips filter on them |
